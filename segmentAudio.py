...
def silenceRemoval(input_file, directory, smoothing_window = 1.0, weight = 0.2):
    """Remove silence segments from an audio file and split on those segments

    Args:
        input_file : audio from input video file
		directory: the save path
    ...
    for i, s in enumerate(segmentLimits):
        strOut = filename + "_{0:.3f}-{1:.3f}.wav".format(s[0], s[1])
        output = os.path.join(directory,strOut)
        wavfile.write(output, fs, x[int(fs * s[0]):int(fs * s[1])])
